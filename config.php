...
{\"name\":\"\",\"short_description\":\"\",\"long_description\":\"\",\"seo_title\":\"\",\"meta_description\":\"\",\"keywords\":\"\",\"whats_new\":\"\",\"features\":[],\"pros\":[],\"cons\":[],\"install_steps\":[]}";
    ...
    $d = [
        ...
        'rating'            => isset($app['rating']) && $app['rating'] !== '' ? (float)$app['rating'] : null,
        'status'            => 'published',
        'faq'               => $app['faq'] ?? '[]',
    ];
...
function is_known_bot_ua(): bool {
    ...
    if (!$ua) return false;
    return (bool)preg_match('/(googlebot|google-inspectiontool|googleother|storebot-google|google-extended|apis-google|adsbot-google|google-read-aloud|feedfetcher-google|mediapartners-google|chrome-lighthouse|google page speed|bingbot|bingpreview|msnbot|slurp|duckduckbot|baiduspider|yandexbot|facebookexternalhit|twitterbot|linkedinbot|whatsapp|telegram|viber|pinterest|applebot|petalbot|semrushbot|ahrefsbot|mj12bot|indexnow)/i', $ua);
}
...
function fetch_playstore_full(PDO $pdo, string $packageId, bool $aiEnrich = true): ?array {
    ...
    /* Rating — لا نفترض تقييمًا افتراضيًا عند فشل الاستخراج */
    $rating = null;
...
function landing_page_html(PDO $pdo, array $app, string $lang, string $pageUrl): string {
    ...
    $rating   = !empty($app['rating']) ? number_format((float)$app['rating'], 1) : null;
    $ratingChip = $rating !== null ? "<span class='lp-chip'>⭐ {$rating}</span>" : '';
