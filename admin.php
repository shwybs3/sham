...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'db_backup' && is_admin()) {
    ...
    header('Content-Disposition: attachment; filename="Apkzilo-backup-' . date('Y-m-d-His') . '.sql"');
    echo "-- Apkzilo database backup — " . date('Y-m-d H:i:s') . "\n";
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'telegram_test' && is_admin()) {
    ...
    $r = telegram_api($pdo, 'sendMessage', [
        'text'       => '✅ اختبار ناجح من لوحة إدارة <b>Apkzilo</b> — بوت تيليجرام يعمل بشكل صحيح!',
        'parse_mode' => 'HTML',
    ]);
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'assistant' && is_admin()) {
    ...
    $prompt = <<<P
أنت مساعد إدارة داخل لوحة تحكم متجر تطبيقات "Apkzilo". لديك قدرة على تنفيذ إجراءات محدّدة فقط عبر إرجاع JSON — لا تنفّذ أي كود، ولا تكتب ملفات، فقط تختار إجراءً من القائمة المسموحة التالية:
...
<head>
  ...
  <title>تسجيل الدخول — Apkzilo admin</title>
...
<head>
  <meta charset="UTF-8"><link rel="icon" type="image/svg+xml" href="<?= h(url("favicon.svg")) ?>"><meta name="theme-color" content="#2563eb"><meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">
  <title><?= h($navLinks[$page]['label'] ?? 'Admin') ?> — Apkzilo</title>
  <meta name="robots" content="noindex">
  <link rel="stylesheet" href="<?= h(asset_url('assets/css/admin.css')) ?>">
</head>
<body>
...
<div class="panel" style="margin-bottom:40px">
  ...
  <p class="form-hint" style="margin-bottom:14px">
    بعد التأكد يدوياً من أن رابط التحميل آمن وسليم، اضغط "تحقق الفريق" لإظهار شارة "رابط تم التحقق من سلامته بواسطة فريق Apkzilo" على صفحة التطبيق وصفحة التحميل.
  </p>
  ...
  <div class="panel">
    ...
    <div class="form-grid">
      ...
      <div class="form-group">
        ...
        <input class="form-input" type="text" name="telegram_channel_id" value="<?= h(get_cfg($pdo,'telegram_channel_id')) ?>" placeholder="@apkzilo_channel أو -100123456789" dir="ltr" style="font-family:var(--f-mono);font-size:12px">
    ...
    <div class="form-group">
      ...
      <input class="form-input" type="url" name="telegram_channel_url" value="<?= h(get_cfg($pdo,'telegram_channel_url')) ?>" placeholder="https://t.me/Apkzilo_channel" dir="ltr">
