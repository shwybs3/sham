...
/* ── AJAX: Auto Setup Domain ── */
if (isset($_GET['ajax']) && $_GET['ajax'] === 'auto_setup_domain' && is_admin()) {
    header('Content-Type: application/json');
    $input   = json_decode(file_get_contents('php://input'), true) ?: [];
    $domain  = preg_replace('/[^a-z0-9\-\.]/', '', strtolower(trim($input['domain'] ?? '')));
    $mode    = in_array($input['mode'] ?? '', ['mirror','redirect','category','standalone']) ? $input['mode'] : 'redirect';
    $autoIdx = (bool)($input['auto_index'] ?? true);
    $autoHtp = (bool)($input['auto_https'] ?? true);
    if (!$domain || !str_contains($domain, '.')) { echo json_encode(['ok'=>false,'error'=>'دومين غير صالح']); exit; }

    $cpUser  = get_cfg($pdo,'cpanel_user','yassqfkf');
    $docRoot = "/home/{$cpUser}/{$domain}";
    $steps   = [];

    // Step 1: Register in domains table
    try {
        $ex = $pdo->prepare("SELECT id FROM domains WHERE full_domain=?")->execute([$domain]);
        $existId = $pdo->query("SELECT id FROM domains WHERE full_domain=".($pdo->quote($domain)))->fetchColumn();
        if ($existId) {
            $pdo->prepare("UPDATE domains SET doc_root=?,site_mode=?,status='active',updated_at=NOW() WHERE id=?")
                ->execute([$docRoot,$mode,$existId]);
        } else {
            $pdo->prepare("INSERT INTO domains (full_domain,doc_root,site_mode,status,created_at) VALUES (?,?,?,'pending',NOW())")
                ->execute([$domain,$docRoot,$mode]);
        }
        $steps[] = ['ok'=>true,'msg'=>"حُفظ الدومين في قاعدة البيانات — المسار: {$docRoot}"];
    } catch (\Throwable $e) { $steps[] = ['ok'=>false,'msg'=>'خطأ DB: '.$e->getMessage()]; }

    // Step 2: cPanel API — add addon domain
    $apiUrl = rtrim(get_cfg($pdo,'cpanel_url',''), '/') ?: 'https://localhost:2083';
    $token  = get_cfg($pdo,'cpanel_api_token','') ?: get_cfg($pdo,'cpanel_api_key','');
    if ($token && $cpUser) {
        $ch = curl_init("{$apiUrl}/execute/AddonDomain/addaddondomain");
        curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>20,CURLOPT_SSL_VERIFYPEER=>false,
            CURLOPT_HTTPHEADER=>["Authorization: cpanel {$cpUser}:{$token}"],CURLOPT_POST=>true,
            CURLOPT_POSTFIELDS=>http_build_query(['newdomain'=>$domain,'subdomain'=>explode('.',$domain)[0],'dir'=>$docRoot])]);
        $res = @json_decode(curl_exec($ch),true); curl_close($ch);
        $ok  = ((int)($res['status']??0)===1) || stripos($res['result'][0]['reason']??'','exist')!==false;
        $steps[] = ['ok'=>$ok,'msg'=>$ok ? "cPanel: تم إضافة الدومين ({$domain})" : 'cPanel: '.($res['result'][0]['reason']??'فشل — تأكد من API Token في الإعدادات')];
    } else {
        $steps[] = ['ok'=>false,'msg'=>'cPanel API غير مُعدَّ — أضف بيانات cPanel في الإعدادات'];
    }

    // Step 3: Install default index.php
    if ($autoIdx) {
        $dir = __DIR__ . '/../' . $domain;
        @mkdir($dir, 0755, true);
        $siteUrl = get_cfg($pdo,'site_url','https://yassota.com');
        $idx = "<?php\n// Auto-generated by yassota hosting manager\n\$mode='{$mode}';\n";
        if ($mode === 'redirect') {
            $idx .= "header('Location: {$siteUrl}/', true, 301); exit;";
        } elseif ($mode === 'mirror') {
            $idx .= "// Mirror mode — symlink or copy yassota files\nheader('Location: {$siteUrl}/', true, 302); exit;";
        } else {
            $idx .= "// Standalone mode\necho '<h1>{$domain}</h1><p>Site under construction.</p>';";
        }
        $idx .= "\n";
        $wrote = @file_put_contents($dir . '/index.php', $idx);
        $steps[] = ['ok'=>(bool)$wrote,'msg'=>$wrote ? "تم تثبيت index.php في {$dir}" : "تعذّر الكتابة في {$dir} — تحقق من صلاحيات PHP"];
    }

    // Step 4: Enable HTTPS via cPanel SSL
    if ($autoHtp && $token && $cpUser) {
        $ch2 = curl_init("{$apiUrl}/execute/SSL/install_ssl");
        curl_setopt_array($ch2,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>20,CURLOPT_SSL_VERIFYPEER=>false,
            CURLOPT_HTTPHEADER=>["Authorization: cpanel {$cpUser}:{$token}"],CURLOPT_POST=>true,
            CURLOPT_POSTFIELDS=>http_build_query(['domain'=>$domain,'enable_sni_without_cert'=>1])]);
        $ssl = @json_decode(curl_exec($ch2),true); curl_close($ch2);
        $sslOk = ((int)($ssl['status']??0)===1);
        $steps[] = ['ok'=>$sslOk,'msg'=>$sslOk ? 'HTTPS: تم تفعيل SSL' : 'HTTPS: يحتاج إعداد يدوي من cPanel → SSL/TLS'];
    }

    // Update domain status
    try { $pdo->prepare("UPDATE domains SET status='active' WHERE full_domain=?")->execute([$domain]); } catch(\Throwable $e) {}

    $allOk = count(array_filter($steps, fn($s)=>$s['ok'])) > 0;
    echo json_encode(['ok'=>$allOk,'steps'=>$steps]);
    exit;
}

/* ── AJAX: WordPress Plugins Search ── */
if (isset($_GET['ajax']) && $_GET['ajax'] === 'wp_plugins_search' && is_admin()) {
    header('Content-Type: application/json');
    $browse  = in_array($_GET['browse'] ?? '', ['popular','featured','new','updated','']) ? ($_GET['browse'] ?? 'popular') : 'popular';
    ...
    if ($search !== '') {
        ...
        $params['request[browse]'] = $browse ?: 'popular';
    }

    $url = 'https://api.wordpress.org/plugins/info/1.2/?' . http_build_query($params);
    $ch  = curl_init($url);
    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_TIMEOUT        => 15,
        CURLOPT_USERAGENT      => 'yassota-admin/1.0',
        CURLOPT_SSL_VERIFYPEER => true,
    ]);
    $raw  = curl_exec($ch);
    $err  = curl_error($ch);
    ...
    if ($err || $code !== 200 || !$raw) {
        echo json_encode(['ok'=>false,'error'=>$err ?: "HTTP {$code}"]);
        exit;
    }
    ...
    foreach (($data['plugins'] ?? []) as $p) {
        $icon = '';
        if (!empty($p['icons']['2x']))  $icon = $p['icons']['2x'];
        elseif (!empty($p['icons']['1x'])) $icon = $p['icons']['1x'];
        elseif (!empty($p['icons']['svg'])) $icon = $p['icons']['svg'];
        $plugins[] = [
            'slug'        => $p['slug'] ?? '',
            'name'        => $p['name'] ?? '',
            ...
            'version'     => $p['version'] ?? '',
            'rating'      => round(($p['rating'] ?? 0) / 20, 1),
            'ratings'     => $p['num_ratings'] ?? 0,
    ...
    echo json_encode(['ok'=>true,'plugins'=>$plugins,'total'=>$data['info']['results'] ?? 0,'page'=>$page,'pages'=>$data['info']['pages'] ?? 1]);
    exit;
}

/* ══════════════════════════════════════════════════════
   AJAX: Domain Broadcast — deploy + index all domains
   ══════════════════════════════════════════════════════ */

/* helper: write a file to a domain docroot via local fs or cPanel API */
function _bc_write_file(string $absPath, string $filename, string $content, array $cpanelCfg): array {
    $target = rtrim($absPath, '/') . '/' . $filename;
    // Attempt direct local write first (same server)
    if (is_dir($absPath) || @mkdir($absPath, 0755, true)) {
        if (@file_put_contents($target, $content) !== false) {
            return ['ok'=>true,'method'=>'local'];
        }
    }
    // Fallback: cPanel UAPI Fileman/save_file_content
    ['url'=>$apiUrl,'user'=>$user,'token'=>$token] = $cpanelCfg;
    if (!$apiUrl || !$token || !$user) return ['ok'=>false,'error'=>'لا يمكن الكتابة محلياً ولا cPanel غير مُعدَّ'];
    $ch = curl_init(rtrim($apiUrl,'/').'/execute/Fileman/save_file_content');
    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER=>true, CURLOPT_TIMEOUT=>20, CURLOPT_SSL_VERIFYPEER=>false,
        CURLOPT_HTTPHEADER=>["Authorization: cpanel {$user}:{$token}"],
        CURLOPT_POST=>true,
        CURLOPT_POSTFIELDS=>json_encode(['dir'=>$absPath,'filename'=>$filename,'content'=>$content,'charset'=>'utf-8']),
    ]);
    $r = @json_decode(curl_exec($ch), true); curl_close($ch);
    if ((int)($r['status']??0)===1) return ['ok'=>true,'method'=>'cpanel'];
    return ['ok'=>false,'error'=>$r['errors'][0] ?? 'cPanel Fileman error'];
}

if (isset($_GET['ajax']) && $_GET['ajax'] === 'broadcast_deploy_one' && is_admin()) {
    header('Content-Type: application/json');
    $input    = json_decode(file_get_contents('php://input'), true) ?: [];
    $domId    = (int)($input['id'] ?? 0);

    $dom = null;
    try { $s=$pdo->prepare("SELECT * FROM domains WHERE id=? LIMIT 1"); $s->execute([$domId]); $dom=$s->fetch(PDO::FETCH_ASSOC); }
    catch (\Throwable $e) {}
    if (!$dom) { echo json_encode(['ok'=>false,'error'=>'النطاق غير موجود']); exit; }

    $docRoot    = $dom['doc_root'] ?? '';
    $srvBase    = rtrim(get_cfg($pdo,'server_doc_root',''), '/') ?: '/home/yassqfkf';
    $mainCfgDir = $srvBase . '/public_html'; // absolute path to main site config
    if (!str_starts_with($docRoot, '/')) $docRoot = $srvBase . '/' . $docRoot;

    $catSlug    = $dom['category_slug'] ?? '';
    $fullDom    = $dom['full_domain'] ?? '';
    $domUrl     = 'https://' . $fullDom;
    $forceHttps = !empty($dom['force_https']);

    $cpCfg = [
        'url'   => rtrim(get_cfg($pdo,'cpanel_url','') ?: get_cfg($pdo,'cpanel_api_url',''), '/') ?: 'https://localhost:2083',
        'user'  => get_cfg($pdo,'cpanel_user',''),
        'token' => get_cfg($pdo,'cpanel_api_token',''),
    ];
    ...
    /* ── 1. Generate index.php with embedded absolute config path ── */
    $tplPath = __DIR__ . '/domain-template/index.php';
    $tplBody = file_exists($tplPath) ? file_get_contents($tplPath) : '<?php echo "Template missing"; ?>';
    // Strip the template's own bootstrap block (lines 1–21) — we replace it with the correct one
    $tplBody = preg_replace('/^<\?php\s*\/\*.*?\*\/.*?if\s*\(\s*!\s*isset\s*\(\s*\$pdo\s*\)\s*\).*?;\s*\}\s*/ms', '', $tplBody, 1);
    $mainCfgEsc = var_export($mainCfgDir, true);
    $indexPhp = "<?php\n" .
        "/* Satellite app-store — auto-deployed by domain-broadcast */\n" .
        "\$_mc = {$mainCfgEsc};\n" .
        "if (file_exists(\$_mc.'/config.local.php')) require_once \$_mc.'/config.local.php';\n" .
        "if (!file_exists(\$_mc.'/config.php')) { http_response_code(503); die('Main config not found at: '.\$_mc); }\n" .
        "require_once \$_mc.'/config.php';\n" .
        "if (!isset(\$pdo)) { http_response_code(503); die('DB not connected'); }\n" .
        $tplBody;

    $r1 = _bc_write_file($docRoot, 'index.php', $indexPhp, $cpCfg);
    $steps[] = ['step'=>'index.php','ok'=>$r1['ok'],'msg'=>$r1['ok']?"✅ index.php نُشر ({$r1['method']})":'❌ '.$r1['error']];

    /* ── 2. Generate sitemap.php with embedded config path ── */
    $sitemapPhp = "<?php\n" .
        "/* Satellite sitemap — domain-broadcast */\n" .
        "\$_mc = {$mainCfgEsc};\n" .
        "if (file_exists(\$_mc.'/config.local.php')) require_once \$_mc.'/config.local.php';\n" .
        "if (!file_exists(\$_mc.'/config.php')) { header('HTTP/1.0 503 Service Unavailable'); exit; }\n" .
        "require_once \$_mc.'/config.php';\n" .
        "if (!isset(\$pdo)) { header('HTTP/1.0 503 Service Unavailable'); exit; }\n" .
        "header('Content-Type: application/xml; charset=utf-8');\n" .
        "\$domUrl = 'https://' . (\$_SERVER['HTTP_HOST'] ?? " . var_export($fullDom,true) . ");\n" .
        "\$catSlug = " . var_export($catSlug, true) . ";\n" .
        "\$where = \"WHERE status='published'\"; \$params = [];\n" .
        "if (\$catSlug) { \$where .= ' AND category_slug=?'; \$params[] = \$catSlug; }\n" .
        "try { \$st=\$pdo->prepare(\"SELECT slug,updated_at FROM apps \$where ORDER BY updated_at DESC LIMIT 1000\");\$st->execute(\$params);\$apps=\$st->fetchAll(); } catch(\\Throwable \$e) { \$apps=[]; }\n" .
        "echo '<?xml version=\"1.0\" encoding=\"UTF-8\"?>' . \"\\n\";\n?>\n" .
        "<urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\">\n" .
        "<url><loc><?php echo \$domUrl; ?>/</loc><changefreq>daily</changefreq><priority>1.0</priority></url>\n" .
        "<?php foreach(\$apps as \$a): ?>\n" .
        "<url><loc><?php echo \$domUrl.'/'.rawurlencode(\$a['slug']); ?></loc>" .
        "<lastmod><?php echo date('Y-m-d',strtotime(\$a['updated_at']??'now')); ?></lastmod>" .
        "<changefreq>weekly</changefreq><priority>0.8</priority></url>\n" .
        "<?php endforeach; ?>\n</urlset>";

    $r2 = _bc_write_file($docRoot, 'sitemap.php', $sitemapPhp, $cpCfg);
    $steps[] = ['step'=>'sitemap.php','ok'=>$r2['ok'],'msg'=>$r2['ok']?"✅ sitemap.php نُشر":'❌ '.$r2['error']];

    /* ── 3. Generate robots.txt ── */
    $robotsTxt = "User-agent: *\nAllow: /\nDisallow: /admin\nSitemap: {$domUrl}/sitemap.php\n";
    $r3 = _bc_write_file($docRoot, 'robots.txt', $robotsTxt, $cpCfg);
    $steps[] = ['step'=>'robots.txt','ok'=>$r3['ok'],'msg'=>$r3['ok']?"✅ robots.txt نُشر":'❌ '.$r3['error']];

    /* ── 4. Generate .htaccess (with HTTPS redirect if enabled) ── */
    $httpsBlock = $forceHttps
        ? "# Force HTTPS\nRewriteCond %{HTTPS} off\nRewriteRule ^(.*)$ https://%{HTTP_HOST}%{REQUEST_URI} [R=301,L]\n\n"
        : '';
    $htaccess = "Options -Indexes\nRewriteEngine On\n\n" .
        $httpsBlock .
        "# Route sitemap.xml → sitemap.php\nRewriteRule ^sitemap\\.xml$ sitemap.php [L]\n\n" .
        "# Front controller\nRewriteCond %{REQUEST_FILENAME} !-f\nRewriteCond %{REQUEST_FILENAME} !-d\nRewriteRule ^ index.php [L]\n";
    $r4 = _bc_write_file($docRoot, '.htaccess', $htaccess, $cpCfg);
    $steps[] = ['step'=>'.htaccess','ok'=>$r4['ok'],'msg'=>$r4['ok']?"✅ .htaccess نُشر".($forceHttps?' (مع فرض HTTPS)':''):'❌ '.$r4['error']];

    /* ── 5. Update DB ── */
    try {
        try { $pdo->exec("ALTER TABLE domains ADD COLUMN last_deployed_at DATETIME NULL"); } catch(\Throwable $e){}
        $pdo->prepare("UPDATE domains SET last_deployed_at=NOW(), status='active' WHERE id=?")->execute([$domId]);
        $steps[] = ['step'=>'db','ok'=>true,'msg'=>'✅ تم تحديث سجل النطاق'];
    } catch (\Throwable $e) {
        $steps[] = ['step'=>'db','ok'=>false,'msg'=>'⚠ DB: '.$e->getMessage()];
    }

    $ok = !in_array(false, array_column($steps, 'ok'), true);
    echo json_encode(['ok'=>$ok,'domain'=>$fullDom,'steps'=>$steps]);
    exit;
}

/* ══════════════════════════════════════════════════════
   AJAX: Create real cPanel subdomain + add to DB + deploy
   ══════════════════════════════════════════════════════ */
if (isset($_GET['ajax']) && $_GET['ajax'] === 'broadcast_create_subdomain' && is_admin()) {
    header('Content-Type: application/json');
    $input      = json_decode(file_get_contents('php://input'), true) ?: [];
    $sub        = preg_replace('/[^a-z0-9\-]/', '', strtolower(trim($input['subdomain'] ?? '')));
    $catSlug    = preg_replace('/[^a-z0-9\-]/', '', strtolower(trim($input['category_slug'] ?? '')));
    $forceHttps = !empty($input['force_https']);
    $autoSsl    = !empty($input['ssl']);
    $autoDeploy = !empty($input['deploy']);

    if (!$sub) { echo json_encode(['ok'=>false,'error'=>'اسم الدومين الفرعي مطلوب']); exit; }

    $cpUrl   = rtrim(get_cfg($pdo,'cpanel_url','') ?: get_cfg($pdo,'cpanel_api_url',''), '/') ?: '';
    $cpUser  = get_cfg($pdo,'cpanel_user','');
    $cpToken = get_cfg($pdo,'cpanel_api_token','');
    $srvBase = rtrim(get_cfg($pdo,'server_doc_root',''), '/') ?: '/home/yassqfkf';
    $mainDom = parse_url(rtrim(get_cfg($pdo,'site_url', defined('SITE_URL')?SITE_URL:''), '/'), PHP_URL_HOST) ?: '';
    $fullDom = $sub . '.' . $mainDom;
    $docRoot = "public_html/{$fullDom}"; // cPanel default for subdomains

    $steps = []; $ok = true; $domId = 0;

    // Ensure domains table + force_https column
    try {
        $pdo->exec("CREATE TABLE IF NOT EXISTS domains (
            ...
            site_mode VARCHAR(50) DEFAULT 'category',
    ...
    $existId = (int)($pdo->query("SELECT id FROM domains WHERE full_domain=".$pdo->quote($fullDom))->fetchColumn() ?: 0);

    // 1. Create in cPanel (attempt even if in DB — might not be created there yet)
    if ($cpUrl && $cpUser && $cpToken) {
        $ch = curl_init("{$cpUrl}/execute/SubDomain/addsubdomain");
        curl_setopt_array($ch,[
        ...
        $resp = curl_exec($ch); $cErr=curl_error($ch); $code=curl_getinfo($ch,CURLINFO_HTTP_CODE); curl_close($ch);
        if ($cErr) {
            $steps[] = ['step'=>'cpanel','ok'=>false,'msg'=>"⚠ cPanel غير متاح: {$cErr} — سيُضاف محلياً"];
        } else {
            $r = @json_decode($resp, true);
            $cpOk = (int)($r['result']['status'] ?? $r['status'] ?? 0) === 1;
            if (!$cpOk) {
                $errMsg = $r['result']['errors'][0] ?? $r['errors'][0] ?? "HTTP {$code}";
                $alreadyEx = stripos((string)$errMsg,'exist')!==false || stripos((string)$errMsg,'already')!==false;
                $steps[] = ['step'=>'cpanel','ok'=>$alreadyEx,'msg'=>$alreadyEx
                    ? "✅ الدومين موجود مسبقاً في cPanel"
                    : "❌ cPanel: {$errMsg}"];
                if (!$alreadyEx) $ok = false;
            } else {
                $steps[] = ['step'=>'cpanel','ok'=>true,'msg'=>"✅ تم إنشاء {$fullDom} في cPanel"];
            }
        }
    } else {
        $steps[] = ['step'=>'cpanel','ok'=>true,'msg'=>"⚠ cPanel غير مُعدَّ — تم التسجيل محلياً فقط"];
    }

    // 2. Add/update in domains DB
    try {
        if ($existId) {
            $pdo->prepare("UPDATE domains SET category_slug=?, doc_root=?, force_https=?, site_mode='category' WHERE id=?")
                ->execute([$catSlug ?: null, $docRoot, $forceHttps ? 1 : 0, $existId]);
            $domId = $existId;
            $steps[] = ['step'=>'db','ok'=>true,'msg'=>'✅ تم تحديث الدومين في قاعدة البيانات'];
        } else {
            $pdo->prepare(
                "INSERT INTO domains (full_domain,category_slug,site_mode,status,doc_root,force_https) VALUES (?,?,'category','pending',?,?)"
            )->execute([$fullDom, $catSlug ?: null, $docRoot, $forceHttps ? 1 : 0]);
            $domId = (int)$pdo->lastInsertId();
            $steps[] = ['step'=>'db','ok'=>true,'msg'=>'✅ تم إضافة الدومين لقاعدة البيانات'];
        }
    } catch (\Throwable $e) {
        $steps[] = ['step'=>'db','ok'=>false,'msg'=>'❌ DB: '.$e->getMessage()]; $ok = false;
    }

    // 3. Optional: trigger AutoSSL (non-blocking)
    if ($autoSsl && $cpUrl && $cpToken) {
        $ch = curl_init("{$cpUrl}/execute/SSL/start_autossl_check");
        ...
        curl_exec($ch); $sslCode=curl_getinfo($ch,CURLINFO_HTTP_CODE); curl_close($ch);
        $steps[] = ['step'=>'ssl','ok'=>$sslCode<400,'msg'=>$sslCode<400
            ? '✅ تم طلب AutoSSL — سيُثبَّت SSL خلال دقائق'
            : "⚠ AutoSSL: HTTP {$sslCode}"];
    }

    // 4. Auto-deploy satellite files if requested
    if ($autoDeploy && $domId && $ok) {
        // Build absolute docroot for local write
        $absDocRoot = str_starts_with($docRoot,'/') ? $docRoot : $srvBase.'/'.$docRoot;
        ...
        $tplBody = preg_replace('/^<\?php\s*\/\*.*?\*\/.*?if\s*\(\s*!\s*isset\s*\(\s*\$pdo\s*\)\s*\).*?;\s*\}\s*/ms', '', $tplBody, 1);
        $indexPhp = "<?php\n\$_mc={$mainCfgEsc};\nif(file_exists(\$_mc.'/config.local.php'))require_once \$_mc.'/config.local.php';\nrequire_once \$_mc.'/config.php';\nif(!isset(\$pdo)){http_response_code(503);die('DB not connected');}\n" . $tplBody;
        $sitemapPhp = "<?php\n\$_mc={$mainCfgEsc};\nif(file_exists(\$_mc.'/config.local.php'))require_once \$_mc.'/config.local.php';\nrequire_once \$_mc.'/config.php';\nif(!isset(\$pdo)){header('HTTP/1.0 503');exit;}\nheader('Content-Type: application/xml; charset=utf-8');\n\$domUrl='https://'.\$_SERVER['HTTP_HOST'];\n\$catSlug=".var_export($catSlug,true).";\n\$where=\"WHERE status='published'\";\$params=[];\nif(\$catSlug){\$where.=' AND category_slug=?';\$params[]=\$catSlug;}\ntry{\$st=\$pdo->prepare(\"SELECT slug,updated_at FROM apps \$where ORDER BY updated_at DESC LIMIT 1000\");\$st->execute(\$params);\$apps=\$st->fetchAll();}catch(\\Throwable \$e){\$apps=[];}\necho '<?xml version=\"1.0\" encoding=\"UTF-8\"?>'.\"\\n\";\n?>\n<urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\">\n<url><loc><?php echo \$domUrl;?>/</loc><changefreq>daily</changefreq><priority>1.0</priority></url>\n<?php foreach(\$apps as \$a):?>\n<url><loc><?php echo \$domUrl.'/'.rawurlencode(\$a['slug']);?></loc><lastmod><?php echo date('Y-m-d',strtotime(\$a['updated_at']??'now'));?></lastmod><changefreq>weekly</changefreq><priority>0.8</priority></url>\n<?php endforeach;?>\n</urlset>";
        $robotsTxt = "User-agent: *\nAllow: /\nDisallow: /admin\nSitemap: https://{$fullDom}/sitemap.php\n";
        $httpsBlock = $forceHttps ? "# Force HTTPS\nRewriteCond %{HTTPS} off\nRewriteRule ^(.*)$ https://%{HTTP_HOST}%{REQUEST_URI} [R=301,L]\n\n" : '';
        $htaccess   = "Options -Indexes\nRewriteEngine On\n\n{$httpsBlock}RewriteRule ^sitemap\\.xml$ sitemap.php [L]\nRewriteCond %{REQUEST_FILENAME} !-f\nRewriteCond %{REQUEST_FILENAME} !-d\nRewriteRule ^ index.php [L]\n";

        $cpCfg = ['url'=>$cpUrl,'user'=>$cpUser,'token'=>$cpToken];
        foreach ([['index.php',$indexPhp],['sitemap.php',$sitemapPhp],['robots.txt',$robotsTxt],['.htaccess',$htaccess]] as [$fn,$ct]) {
            $wr = _bc_write_file($absDocRoot, $fn, $ct, $cpCfg);
            $steps[] = ['step'=>"deploy:{$fn}",'ok'=>$wr['ok'],'msg'=>$wr['ok']?"✅ {$fn} نُشر":'❌ '.$wr['error']];
        }
        try { $pdo->prepare("UPDATE domains SET last_deployed_at=NOW(),status='active' WHERE id=?")->execute([$domId]); } catch(\Throwable $e){}
    }

    $ok = $ok && !in_array(false, array_column($steps,'ok'));
    echo json_encode(['ok'=>$ok,'domain'=>$fullDom,'doc_root'=>$docRoot,'dom_id'=>$domId,'steps'=>$steps]);
    exit;
}

/* ══════════════════════════════════════════════════════
   AJAX: List cPanel subdomains
   ══════════════════════════════════════════════════════ */
if (isset($_GET['ajax']) && $_GET['ajax'] === 'broadcast_list_cpanel_subs' && is_admin()) {
    header('Content-Type: application/json');
    $cpUrl   = rtrim(get_cfg($pdo,'cpanel_url','') ?: get_cfg($pdo,'cpanel_api_url',''), '/');
    $cpUser  = get_cfg($pdo,'cpanel_user','');
    $cpToken = get_cfg($pdo,'cpanel_api_token','');
    if (!$cpUrl || !$cpToken) { echo json_encode(['ok'=>false,'error'=>'cPanel غير مُعدَّ']); exit; }
    $ch = curl_init("{$cpUrl}/execute/SubDomain/listsubdomains");
    curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>15,CURLOPT_SSL_VERIFYPEER=>false,
        CURLOPT_HTTPHEADER=>["Authorization: cpanel {$cpUser}:{$cpToken}"]]);
    $resp = curl_exec($ch); curl_close($ch);
    $r = @json_decode($resp,true);
    $subs = $r['data'] ?? $r['result']['data'] ?? [];
    echo json_encode(['ok'=>true,'subdomains'=>$subs]);
    exit;
}

/* ══════════════════════════════════════════════════════
   AJAX: Force HTTPS toggle for a domain
   ══════════════════════════════════════════════════════ */
if (isset($_GET['ajax']) && $_GET['ajax'] === 'broadcast_set_https' && is_admin()) {
    header('Content-Type: application/json');
    $input = json_decode(file_get_contents('php://input'), true) ?: [];
    $domId = (int)($input['id'] ?? 0);
    $val   = !empty($input['force_https']) ? 1 : 0;
    try { $pdo->exec("ALTER TABLE domains ADD COLUMN force_https TINYINT(1) DEFAULT 1"); } catch(\Throwable $e){}
    try {
        $pdo->prepare("UPDATE domains SET force_https=? WHERE id=?")->execute([$val, $domId]);
        echo json_encode(['ok'=>true,'force_https'=>$val]);
    } catch(\Throwable $e) { echo json_encode(['ok'=>false,'error'=>$e->getMessage()]); }
    exit;
}

if (isset($_GET['ajax']) && $_GET['ajax'] === 'broadcast_indexnow' && is_admin()) {
    header('Content-Type: application/json');
    $input  = json_decode(file_get_contents('php://input'), true) ?: [];
    $domIds = array_map('intval', (array)($input['ids'] ?? []));
    $inKey  = get_cfg($pdo, 'indexnow_key', '');
    if (!$inKey) { echo json_encode(['ok'=>false,'error'=>'أدخل مفتاح IndexNow في الإعدادات (indexnow_key)']); exit; }

    $results = [];
    foreach ($domIds as $id) {
        try {
            $dom = $pdo->prepare("SELECT full_domain FROM domains WHERE id=?"); $dom->execute([$id]);
            $row = $dom->fetch(PDO::FETCH_ASSOC);
            if (!$row) continue;
            $host = $row['full_domain'];
            $ch = curl_init('https://api.indexnow.org/indexnow');
            curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>10,CURLOPT_SSL_VERIFYPEER=>false,
                CURLOPT_POST=>true,CURLOPT_HTTPHEADER=>['Content-Type: application/json'],
                CURLOPT_POSTFIELDS=>json_encode([
                    'host'=>$host,'key'=>$inKey,'keyLocation'=>"https://{$host}/{$inKey}.txt",
                    'urlList'=>["https://{$host}/","https://{$host}/sitemap.php"]
                ])]);
            $resp = curl_exec($ch);
            $code = curl_getinfo($ch,CURLINFO_HTTP_CODE); curl_close($ch);
            $results[] = ['domain'=>$host,'code'=>$code,'ok'=>$code<300];
        } catch (\Throwable $e) { $results[] = ['domain'=>$id,'ok'=>false,'error'=>$e->getMessage()]; }
    }
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'broadcast_ping_sitemaps' && is_admin()) {
    header('Content-Type: application/json');
    $input  = json_decode(file_get_contents('php://input'), true) ?: [];
    $domIds = array_map('intval', (array)($input['ids'] ?? []));
    $results = [];
    foreach ($domIds as $id) {
        try {
            ...
            $row = $dom->fetch(PDO::FETCH_ASSOC);
            if (!$row) continue;
            $sitemap = rawurlencode("https://{$row['full_domain']}/sitemap.php");
            // Ping Google
            $ch = curl_init("https://www.google.com/ping?sitemap={$sitemap}");
            curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>8,CURLOPT_SSL_VERIFYPEER=>false,CURLOPT_USERAGENT=>'yassota/1.0']);
            curl_exec($ch); $gCode=curl_getinfo($ch,CURLINFO_HTTP_CODE); curl_close($ch);
            // Ping Bing
            $ch = curl_init("https://www.bing.com/ping?sitemap={$sitemap}");
            curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>8,CURLOPT_SSL_VERIFYPEER=>false,CURLOPT_USERAGENT=>'yassota/1.0']);
            curl_exec($ch); $bCode=curl_getinfo($ch,CURLINFO_HTTP_CODE); curl_close($ch);
            $results[] = ['domain'=>$row['full_domain'],'google'=>$gCode,'bing'=>$bCode,'ok'=>($gCode<300||$bCode<300)];
        } catch (\Throwable $e) { $results[] = ['domain'=>$id,'ok'=>false,'error'=>$e->getMessage()]; }
    }
    echo json_encode(['ok'=>true,'results'=>$results]);
    exit;
...
$navLinks = [
    ...
    'wordpress-plugins' => ['label'=>'متجر ووردبريس بلاقنز', 'icon'=>'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 14.5v-9l6 4.5-6 4.5z'],
...
$checks = [
    ...
    ['icons',    ($publishedCount > 0 && $hasIcon/$publishedCount >= 0.9) ? 'pass' : ($publishedCount===0 ? 'pass' : 'warn'), 5,
        "أيقونات التطبيقات ({$hasIcon}/{$publishedCount})",
        'الأيقونات تحسّن تجربة المستخدم ومعدل الإقامة.',
        ($publishedCount===0 || $hasIcon/$publishedCount >= 0.9) ? 'ممتاز' : "يوجد ".($publishedCount-$hasIcon)." تطبيق بدون أيقونة."],
...
<div class="panel">
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:10px">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      <button class="btn-edit" onclick="selectAllTools(true)" style="font-size:12px;padding:6px 12px">تحديد الكل</button>
      <button class="btn-edit" onclick="selectAllTools(false)" style="font-size:12px;padding:6px 12px">إلغاء الكل</button>
      <button class="btn-save" onclick="registerSelected()" id="btn-reg-selected" style="font-size:12px;padding:6px 14px">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:middle;margin-left:4px"><path d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3"/></svg>
        تسجيل المحدّد في cPanel
      </button>
    </div>
  </div>
  ...
  <div style="overflow-x:auto">
    <table class="admin-table" style="min-width:560px">
      <thead><tr>
        <th style="width:36px"><input type="checkbox" id="chk-all" onchange="selectAllTools(this.checked)" style="width:16px;height:16px;cursor:pointer"></th>
        <th>الأداة</th>
        <th>النطاق الفرعي</th>
        <th>المسار</th>
        <th>الحالة</th>
      ...
      <tbody>
        ...
        <tr id="tr-<?= $t['slug'] ?>">
          <td><input type="checkbox" class="tool-chk" value="<?= h($t['slug']) ?>" style="width:16px;height:16px;cursor:pointer"></td>
          <td>
            <strong><?= h($t['name']) ?></strong>
            <div style="font-size:11px;color:var(--muted)"><?= h($t['desc']) ?></div>
          </td>
          <td>
            <a href="https://<?= h($sub) ?>/" target="_blank" style="color:var(--primary);font-family:monospace;font-size:13px;direction:ltr;display:inline-block"><?= h($sub) ?></a>
          </td>
          <td><code style="font-family:monospace;font-size:11px;color:var(--muted);direction:ltr">/tools/<?= h($t['slug']) ?>/</code></td>
          <td>
            <span id="status-<?= $t['slug'] ?>" style="font-size:12px;color:var(--muted)">—</span>
          </td>
        </tr>
        <?php endforeach; ?>
      </tbody>
...
function registerSelected() {
  var slugs = [...document.querySelectorAll('.tool-chk:checked')].map(function(c){return c.value;});
  if (!slugs.length) { alert('حدّد أداة واحدة على الأقل'); return; }
  var btn = document.getElementById('btn-reg-selected');
  ...
  function next() {
    ...
    var fd = new FormData(); fd.append('subdomain', slug);
...
<div id="tab-search" class="dm-tab-panel">
  ...
  <!-- Results grid -->
  <div id="dm-results" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px"></div>
  <div id="dm-empty" style="text-align:center;padding:40px;color:var(--muted)">
    <div style="font-size:40px;margin-bottom:12px">🌐</div>
    <p>أدخل اسماً واضغط "بحث" للبدء — سيتم اقتراح مئات التركيبات</p>
  </div>
  <div style="text-align:center;margin-top:24px">
    <button id="dm-load-more" onclick="loadNextBatch()" style="display:none;padding:11px 32px;background:var(--primary);color:#fff;border:none;border-radius:10px;cursor:pointer;font-size:14px;font-weight:600;display:none">⬇ تحميل المزيد</button>
  </div>
</div>
...
/* ── Domain search: name variants + Load More pagination ── */
var dmAllCombos   = [];
var dmComboOffset = 0;
var DM_PAGE_SIZE  = 20;

function generateNameVariants(base) {
  var prefixes = ['get','go','try','the','pro','top','my','best','fast','smart','real','new','one','easy','super','ultra','mega','mini','quick','safe','web','i','app','open','free','live','all','max','info','use'];
  var suffixes = ['app','pro','hub','plus','now','ai','tech','store','zone','online','io','hq','world','net','land','spot','lab','digital','media','soft','api','cloud','base','box','link','point','home','center','place','desk'];
  var variants = [base];
  prefixes.forEach(function(p){ if(p+base !== base) variants.push(p+base); });
  suffixes.forEach(function(s){ if(base+s !== base) variants.push(base+s); });
  return variants;
}
...
function startDomainSearch() {
  ...
  var variants = generateNameVariants(name);
  dmAllCombos = [];
  variants.forEach(function(v){ tlds.forEach(function(t){ dmAllCombos.push({name:v, tld:t}); }); });

  dmComboOffset = 0;
  document.getElementById('dm-results').innerHTML = '';
  document.getElementById('dm-empty').style.display = 'none';
  document.getElementById('dm-load-more').style.display = 'none';
...
function loadNextBatch() {
  var batch = dmAllCombos.slice(dmComboOffset, dmComboOffset + DM_PAGE_SIZE);
  if (!batch.length) return;
  dmComboOffset += batch.length;

  var resultsDiv = document.getElementById('dm-results');
  batch.forEach(function(item) {
    var t = item.tld;
    var cardId = 'dmc-' + item.name.replace(/[^a-z0-9]/g,'') + '-' + t.tld.replace(/\./g,'-');
    var card = document.createElement('div');
    card.id = cardId;
    card.style.cssText = 'background:var(--bg);border:1px solid var(--border);border-radius:10px;padding:12px;transition:border-color .3s';
    card.innerHTML =
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">'
        +'<span style="font-size:15px">'+t.emoji+'</span>'
        +'<span class="avail-badge" style="font-size:10px;color:#94a3b8">⏳</span>'
      +'</div>'
      +'<div style="font-family:monospace;font-weight:800;font-size:12px;color:var(--primary);word-break:break-all;line-height:1.4" dir="ltr">'+item.name+'.'+t.tld+'</div>'
      +'<div style="font-size:10px;color:var(--muted);margin:3px 0">'+t.price+'</div>'
      +'<div class="dm-card-actions" style="margin-top:8px;gap:5px;flex-wrap:wrap;display:none"></div>';
    resultsDiv.appendChild(card);
  });

  var queue = batch.slice(); var running = 0; var PARALLEL = 8;
  function checkNext() {
    while (running < PARALLEL && queue.length) {
      var item = queue.shift(); running++;
      checkTldItem(item, function(){ running--; checkNext(); });
    }
    if (!queue.length && running === 0) {
      ...
      var lm = document.getElementById('dm-load-more');
      var rem = dmAllCombos.length - dmComboOffset;
      if (rem > 0) {
        lm.style.display = 'inline-block';
        lm.textContent = '⬇ تحميل المزيد (' + rem + ' تركيبة متبقية)';
      } else {
...
function checkTldItem(item, done) {
  var t = item.tld;
  var cardId = 'dmc-' + item.name.replace(/[^a-z0-9]/g,'') + '-' + t.tld.replace(/\./g,'-');
  var card = document.getElementById(cardId);
  fetch('admin.php?ajax=domain_check&name='+encodeURIComponent(item.name)+'&tld='+encodeURIComponent(t.tld))
    .then(function(r){return r.json();})
    .then(function(d){
      if (!card) { done(); return; }
      var badge   = card.querySelector('.avail-badge');
      var actions = card.querySelector('.dm-card-actions');
      var svgCheck = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>';
      var svgExt   = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>';
      if (d.status === 'available') {
        card.style.borderColor = '#22c55e'; badge.style.color = '#22c55e'; badge.textContent = '✅ متاح';
        actions.style.display = 'flex';
        actions.innerHTML =
          '<button onclick="reserveDomain(\''+item.name+'\',\''+t.tld+'\',\''+t.type+'\',\''+escHtml(t.price)+'\')" '
          +'style="flex:1;background:#22c55e;color:#fff;border:none;border-radius:6px;padding:6px 2px;cursor:pointer;font-size:11px;display:flex;align-items:center;justify-content:center;gap:3px">'
          +svgCheck+'احجز</button>'
          +(t.reg ? '<a href="https://'+t.reg+'" target="_blank" style="flex:1;background:#3b82f6;color:#fff;border-radius:6px;padding:6px 2px;font-size:11px;text-align:center;text-decoration:none;display:flex;align-items:center;justify-content:center;gap:3px">'+svgExt+'سجّل</a>' : '');
      } else if (d.status === 'taken') {
        card.style.borderColor = '#ef4444'; badge.style.color = '#ef4444'; badge.textContent = '❌ مأخوذ';
      } else {
        card.style.borderColor = '#f59e0b'; badge.style.color = '#f59e0b'; badge.textContent = '⚠ غير معروف';
        actions.style.display = 'flex';
        actions.innerHTML = '<button onclick="reserveDomain(\''+item.name+'\',\''+t.tld+'\',\''+t.type+'\',\''+escHtml(t.price)+'\')" style="width:100%;background:#f59e0b;color:#fff;border:none;border-radius:6px;padding:6px 0;cursor:pointer;font-size:11px">احجز على أي حال</button>';
      }
      done();
    }).catch(function(){ done(); });
}
...
elseif ($page === 'wordpress-plugins'):
?>
<div class="admin-header">
  <h1>متجر إضافات ووردبريس</h1>
  <p style="color:var(--muted);font-size:13px;margin-top:4px">تصفّح أشهر إضافات WordPress.org — ابحث، استكشف، احفظ ما يناسبك</p>
</div>

<!-- Category chips -->
<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:18px;align-items:center">
  <input id="wp-search-input" type="text" class="form-input" placeholder="ابحث عن إضافة..." style="flex:1;min-width:200px;max-width:340px" onkeydown="if(event.key==='Enter')wpSearch()">
  <button onclick="wpSearch()" class="btn-primary" style="padding:9px 20px;display:flex;align-items:center;gap:6px">
    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>بحث
  </button>
</div>
<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:20px" id="wp-browse-chips">
  <?php foreach(['popular'=>'🔥 الأكثر شيوعاً','featured'=>'⭐ مختارة','new'=>'🆕 جديدة','updated'=>'🔄 محدّثة'] as $k=>$l): ?>
  <button class="wp-chip" data-browse="<?= $k ?>" onclick="wpBrowse('<?= $k ?>',this)" style="padding:7px 16px;border-radius:20px;border:1px solid var(--border);background:<?= $k==='popular'?'var(--primary);color:#fff':'var(--bg);color:var(--text)' ?>;cursor:pointer;font-size:13px"><?= $l ?></button>
  <?php endforeach; ?>
</div>

<!-- Results -->
<div id="wp-results" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px"></div>
<div id="wp-empty" style="text-align:center;padding:48px;color:var(--muted)">
  <div style="font-size:48px;margin-bottom:14px">🔌</div>
  <p style="font-size:15px">اضغط على أي تصنيف أو ابحث عن إضافة</p>
</div>
<div id="wp-loading" style="display:none;text-align:center;padding:30px;color:var(--muted)">
  <div style="font-size:32px;animation:spin 1s linear infinite;display:inline-block">⏳</div>
  <p>جارٍ التحميل...</p>
</div>
<div style="text-align:center;margin-top:24px">
  <button id="wp-load-more" onclick="wpLoadMore()" style="display:none;padding:11px 32px;background:var(--primary);color:#fff;border:none;border-radius:10px;cursor:pointer;font-size:14px;font-weight:600">تحميل المزيد</button>
</div>

<style>
@keyframes spin{to{transform:rotate(360deg)}}
.wp-plugin-card{background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:18px;display:flex;flex-direction:column;gap:10px;transition:box-shadow .2s}
.wp-plugin-card:hover{box-shadow:0 4px 20px rgba(0,0,0,.1)}
.wp-plugin-card .wp-icon{width:56px;height:56px;border-radius:10px;object-fit:cover;background:var(--bg);flex-shrink:0}
.wp-stars{color:#f59e0b;font-size:13px;letter-spacing:1px}
.wp-badge{display:inline-block;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:600;background:var(--bg);color:var(--muted);border:1px solid var(--border)}
</style>

<script>
var wpCurrentBrowse = 'popular';
var wpCurrentSearch = '';
var wpCurrentPage   = 1;
var wpTotalPages    = 1;

function wpBrowse(browse, el) {
  wpCurrentBrowse = browse;
  wpCurrentSearch = '';
  wpCurrentPage   = 1;
  document.getElementById('wp-search-input').value = '';
  document.querySelectorAll('.wp-chip').forEach(function(b){
...
function wpSearch() {
  var q = document.getElementById('wp-search-input').value.trim();
  wpCurrentSearch = q;
  wpCurrentPage   = 1;
  document.querySelectorAll('.wp-chip').forEach(function(b){
    b.style.background='var(--bg)'; b.style.color='var(--text)';
...
function wpFetch(append) {
  var lm = document.getElementById('wp-load-more');
  var ld = document.getElementById('wp-loading');
  var em = document.getElementById('wp-empty');
  if (!append) { document.getElementById('wp-results').innerHTML=''; lm.style.display='none'; }
  em.style.display='none'; ld.style.display='block';

  var url = 'admin.php?ajax=wp_plugins_search&p='+wpCurrentPage;
  if (wpCurrentSearch) url += '&search='+encodeURIComponent(wpCurrentSearch);
  else url += '&browse='+encodeURIComponent(wpCurrentBrowse);

  fetch(url).then(function(r){return r.json();}).then(function(d){
    ld.style.display='none';
    if (!d.ok || !d.plugins || !d.plugins.length) {
      if (!append) em.style.display='block';
      return;
    }
    wpTotalPages = d.pages || 1;
    var grid = document.getElementById('wp-results');
    d.plugins.forEach(function(p){ grid.appendChild(wpCard(p)); });
    if (wpCurrentPage < wpTotalPages) {
      lm.style.display='inline-block';
    } else {
      lm.style.display='none';
    }
  }).catch(function(){ ld.style.display='none'; em.style.display='block'; });
}

function wpCard(p) {
  var el = document.createElement('div');
  el.className = 'wp-plugin-card';
  var stars = '';
  var r = p.rating;
  for (var i=1;i<=5;i++) stars += (i<=Math.floor(r)?'★':(i-r<1&&i-r>0?'⯨':'☆'));
  var dl = p.downloads >= 1000000 ? (p.downloads/1000000).toFixed(1)+'M'
         : p.downloads >= 1000 ? Math.round(p.downloads/1000)+'K' : p.downloads;
  var iconHtml = p.icon
    ? '<img src="'+escH(p.icon)+'" class="wp-icon" loading="lazy" onerror="this.style.display=\'none\'">'
    : '<div class="wp-icon" style="display:flex;align-items:center;justify-content:center;font-size:26px">🔌</div>';

  el.innerHTML =
    '<div style="display:flex;gap:12px;align-items:flex-start">'
      +iconHtml
      +'<div style="flex:1;min-width:0">'
        +'<div style="font-weight:700;font-size:15px;line-height:1.3">'+escH(p.name)+'</div>'
        +'<div style="font-size:12px;color:var(--muted);margin-top:2px">'+escH(p.author)+'</div>'
      +'</div>'
    +'</div>'
    +'<p style="font-size:13px;color:var(--text);margin:0;line-height:1.5;-webkit-line-clamp:3;display:-webkit-box;-webkit-box-orient:vertical;overflow:hidden">'+escH(p.description)+'</p>'
    +'<div style="display:flex;flex-wrap:wrap;gap:6px;align-items:center">'
      +'<span class="wp-stars">'+stars+'</span>'
      +'<span style="font-size:12px;color:var(--muted)">('+p.ratings.toLocaleString()+')</span>'
      +'<span class="wp-badge">⬇ '+dl+'</span>'
      +(p.version?'<span class="wp-badge">v'+escH(p.version)+'</span>':'')
    +'</div>'
    +'<a href="'+escH(p.url)+'" target="_blank" style="display:flex;align-items:center;justify-content:center;gap:6px;padding:8px;background:var(--primary);color:#fff;border-radius:8px;text-decoration:none;font-size:13px;font-weight:600">'
      +'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>'
      +'عرض على WordPress.org</a>';
  return el;
}

function escH(s){ return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }

// Auto-load popular on page ready
document.addEventListener('DOMContentLoaded', function(){ wpFetch(); });
</script>

<?php
/* ═══════════════════════════════════════════════════════════
   DOMAIN BROADCAST — نشر وفهرسة متعدد الدومينات
   ═══════════════════════════════════════════════════════════ */
elseif ($page === 'domain-broadcast'):

// Ensure last_deployed_at column exists
try { $pdo->exec("ALTER TABLE domains ADD COLUMN last_deployed_at DATETIME NULL"); } catch(\Throwable $e){}

$bcDomains = [];
try {
    $bcDomains = $pdo->query(
        "SELECT id, full_domain, site_mode, category_slug, status, doc_root,
                last_deployed_at, created_at
         FROM domains ORDER BY status DESC, full_domain ASC"
    )->fetchAll(PDO::FETCH_ASSOC);
} catch (\Throwable $e) { $bcDomains = []; }

$mainUrl     = rtrim(get_cfg($pdo,'site_url', SITE_URL), '/');
$inKey       = get_cfg($pdo,'indexnow_key','');
$srvDocRoot  = rtrim(get_cfg($pdo,'server_doc_root',''), '/');
$deployedCount = count(array_filter($bcDomains, fn($d)=>!empty($d['last_deployed_at'])));
$totalCount    = count($bcDomains);
?>
<style>
.bc-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:16px;margin-top:20px}
.bc-card{background:var(--surface);border:2px solid var(--border);border-radius:14px;padding:18px;transition:border-color .2s}
.bc-card.deployed{border-color:rgba(34,197,94,.4)}
.bc-card.pending{border-color:rgba(245,158,11,.4)}
.bc-card-top{display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:12px}
.bc-domain{font-family:monospace;font-weight:800;font-size:15px;color:var(--primary);word-break:break-all}
.bc-badge{font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;white-space:nowrap}
.bc-badge.ok{background:rgba(34,197,94,.15);color:#22c55e}
.bc-badge.warn{background:rgba(245,158,11,.15);color:#f59e0b}
.bc-badge.none{background:rgba(148,163,184,.15);color:#94a3b8}
.bc-meta{font-size:12px;color:var(--muted);display:flex;flex-wrap:wrap;gap:8px;margin-bottom:14px}
.bc-actions{display:flex;gap:6px;flex-wrap:wrap}
.bc-log{margin-top:12px;border-radius:8px;background:var(--bg);padding:10px;font-size:11px;font-family:monospace;display:none;max-height:180px;overflow-y:auto}
.bc-log-line{padding:2px 0;border-bottom:1px solid var(--border)}
.bc-log-line:last-child{border-bottom:none}
.bc-progress{height:4px;border-radius:2px;background:var(--border);margin:12px 0;overflow:hidden;display:none}
.bc-progress-bar{height:100%;background:linear-gradient(90deg,#3b82f6,#22c55e);border-radius:2px;width:0;transition:width .3s}
</style>

<div class="admin-header">
  <h1>📡 نشر متعدد الدومينات</h1>
  <p style="color:var(--muted);font-size:13px;margin-top:4px">
    انشر موقع متجر التطبيقات على جميع دوماناتك المسجلة — كل دومين يُفهرس بشكل مستقل ويقود الزوار للموقع الأم
  </p>
</div>

<!-- ── Stats ── -->
<div style="display:flex;gap:14px;flex-wrap:wrap;margin-bottom:24px">
  <div class="panel" style="flex:1;min-width:120px;text-align:center;padding:16px">
    <div style="font-size:32px;font-weight:800;color:var(--primary)"><?= $totalCount ?></div>
    <div style="font-size:12px;color:var(--muted)">دومين مسجل</div>
  </div>
  <div class="panel" style="flex:1;min-width:120px;text-align:center;padding:16px">
    <div style="font-size:32px;font-weight:800;color:#22c55e"><?= $deployedCount ?></div>
    <div style="font-size:12px;color:var(--muted)">تم نشره</div>
  </div>
  <div class="panel" style="flex:1;min-width:120px;text-align:center;padding:16px">
    <div style="font-size:32px;font-weight:800;color:#f59e0b"><?= $totalCount - $deployedCount ?></div>
    <div style="font-size:12px;color:var(--muted)">لم يُنشر بعد</div>
  </div>
</div>

<!-- ── Broadcast Control Panel ── -->
<div class="panel" style="border-right:4px solid var(--primary)">
  <h3 style="margin:0 0 14px;font-size:15px">🚀 نشر جماعي لجميع الدومينات</h3>
  <p style="font-size:13px;color:var(--muted);margin:0 0 16px">
    ينشر على كل دومين: <code>index.php</code> (قائمة التطبيقات) + <code>sitemap.php</code> + <code>robots.txt</code> + <code>.htaccess</code> — يقرأ من نفس قاعدة البيانات تلقائياً.
  </p>

  <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin-bottom:14px">
    <label style="display:flex;align-items:center;gap:7px;font-size:13px;cursor:pointer">
      <input type="checkbox" id="bc-select-all" onchange="bcSelectAll(this.checked)"> <strong>تحديد الكل</strong>
    </label>
    <span style="color:var(--muted);font-size:12px" id="bc-sel-count">0 دومين محدد</span>
  </div>

  <div id="bc-progress" class="bc-progress">
    <div id="bc-progress-bar" class="bc-progress-bar"></div>
  </div>

  <div style="display:flex;gap:10px;flex-wrap:wrap">
    <button onclick="bcDeploySelected()" class="btn-primary" style="display:flex;align-items:center;gap:7px">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
      نشر المحدد
    </button>
    <button onclick="bcIndexNowSelected()" style="padding:10px 18px;background:#6366f1;color:#fff;border:none;border-radius:10px;cursor:pointer;display:flex;align-items:center;gap:7px;font-size:13px">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
      فهرسة IndexNow
    </button>
    <button onclick="bcPingSitemaps()" style="padding:10px 18px;background:#0ea5e9;color:#fff;border:none;border-radius:10px;cursor:pointer;display:flex;align-items:center;gap:7px;font-size:13px">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.82 19.79 19.79 0 01.12 1.2 2 2 0 012.1 0h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 7.09a16 16 0 006 6l.45-.45a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 14.92z"/></svg>
      Ping Google & Bing
    </button>
    <a href="admin.php?page=domain-manager" style="padding:10px 18px;background:var(--bg);border:1px solid var(--border);border-radius:10px;cursor:pointer;display:flex;align-items:center;gap:7px;font-size:13px;color:var(--text);text-decoration:none">
      + إضافة دومين جديد
    </a>
  </div>

  <div id="bc-global-log" style="margin-top:14px;display:grid;gap:6px"></div>
</div>

<!-- ── How it works ── -->
<div class="panel" style="margin-top:20px;background:linear-gradient(135deg,rgba(99,102,241,.05),rgba(34,197,94,.05));border:1px solid rgba(99,102,241,.2)">
  <h4 style="margin:0 0 10px;font-size:13px;color:var(--primary)">⚙️ كيف يعمل؟</h4>
  <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px;font-size:12px;color:var(--muted)">
    <div>📁 <strong>نفس قاعدة البيانات</strong> — كل الدومينات تقرأ من DB الموقع الرئيسي تلقائياً</div>
    <div>🔗 <strong>روابط التطبيقات → الموقع الرئيسي</strong> — الزوار ينتقلون لـ <?= h($mainUrl) ?></div>
    <div>🗺️ <strong>Sitemap مستقل لكل دومين</strong> — Google يفهرس كل دومين على حدة</div>
    <div>📡 <strong>فهرسة IndexNow</strong> — إشعار فوري لـ Google/Bing عند النشر</div>
    <div>🏷️ <strong>تصفية بالقسم</strong> — كل دومين يعرض تطبيقات قسمه المحدد</div>
    <div>📱 <strong>تصميم احترافي داكن</strong> — قالب جاهز بتأثيرات بصرية راقية</div>
  </div>
</div>

<!-- ── Domain cards ── -->
<?php if (empty($bcDomains)): ?>
<div style="text-align:center;padding:48px;color:var(--muted)">
  <div style="font-size:48px;margin-bottom:14px">🌐</div>
  <p style="margin-bottom:16px">لا توجد دومينات مسجلة بعد</p>
  <a href="admin.php?page=domain-manager" class="btn-primary">+ إضافة دومين</a>
</div>
<?php else: ?>
<div class="bc-grid">
  <?php foreach ($bcDomains as $dom):
    $deployed   = !empty($dom['last_deployed_at']);
    $badgeCls   = $deployed ? 'ok' : 'none';
    $badgeTxt   = $deployed ? 'نُشر' : 'لم يُنشر';
    $domUrl     = 'https://' . h($dom['full_domain']);
    $catTxt     = $dom['category_slug'] ? '📂 '.$dom['category_slug'] : '📱 جميع التطبيقات';
    $modeMap    = ['mirror'=>'مرآة','redirect'=>'إعادة توجيه','category'=>'قسم','standalone'=>'مستقل'];
    $modeTxt    = $modeMap[$dom['site_mode'] ?? ''] ?? $dom['site_mode'] ?? '—';
    $deployedAt = $dom['last_deployed_at'] ? date('Y-m-d H:i', strtotime($dom['last_deployed_at'])) : 'لم يُنشر';
  ?>
  <div class="bc-card <?= $deployed ? 'deployed' : 'pending' ?>" id="bc-card-<?= $dom['id'] ?>">
    <div class="bc-card-top">
      <label style="display:flex;gap:8px;align-items:flex-start;cursor:pointer;flex:1">
        <input type="checkbox" class="bc-chk" value="<?= $dom['id'] ?>" onchange="bcUpdateCount()">
        <span class="bc-domain"><?= h($dom['full_domain']) ?></span>
      </label>
      <span class="bc-badge <?= $badgeCls ?>"><?= $badgeTxt ?></span>
    </div>

    <div class="bc-meta">
      <span>📂 <?= $dom['category_slug'] ? h($dom['category_slug']) : 'جميع' ?></span>
      <span>⚙️ <?= h($modeTxt) ?></span>
      <span>🕐 <?= $deployedAt ?></span>
      <?php if ($dom['doc_root']): ?>
      <span title="<?= h($dom['doc_root']) ?>" style="font-family:monospace;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">📁 <?= h(basename($dom['doc_root'])) ?></span>
      <?php endif; ?>
    </div>

    <div class="bc-actions">
      <button onclick="bcDeployOne(<?= $dom['id'] ?>)" class="btn-sm" style="background:var(--primary);color:#fff;display:flex;align-items:center;gap:4px">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>نشر
      </button>
      <button onclick="bcIndexNowOne(<?= $dom['id'] ?>)" class="btn-sm" style="background:#6366f1;color:#fff;display:flex;align-items:center;gap:4px">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>IndexNow
      </button>
      <a href="<?= $domUrl ?>" target="_blank" class="btn-sm" style="display:flex;align-items:center;gap:4px;text-decoration:none">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>معاينة
      </a>
      <a href="<?= $domUrl ?>/sitemap.php" target="_blank" class="btn-sm" style="display:flex;align-items:center;gap:4px;text-decoration:none">🗺️ Sitemap</a>
    </div>

    <div class="bc-log" id="bc-log-<?= $dom['id'] ?>"></div>
  </div>
  <?php endforeach; ?>
</div>
<?php endif; ?>
...
function bcUpdateCount() {
  var n = document.querySelectorAll('.bc-chk:checked').length;
  document.getElementById('bc-sel-count').textContent = n + ' دومين محدد';
}
...
function bcSelectedIds() {
  return [...document.querySelectorAll('.bc-chk:checked')].map(function(c){return parseInt(c.value);});
}

function bcSetCardState(id, state, msg) {
  var card = document.getElementById('bc-card-'+id);
  var log  = document.getElementById('bc-log-'+id);
  if (!card || !log) return;
  log.style.display = 'block';
  var ln = document.createElement('div');
  ln.className = 'bc-log-line';
  ln.style.color = state==='ok'?'#22c55e':state==='err'?'#ef4444':'var(--muted)';
  ln.textContent = msg;
  log.appendChild(ln);
  log.scrollTop = log.scrollHeight;
}

function bcDeployOne(id) {
  var log = document.getElementById('bc-log-'+id);
  if (log) { log.innerHTML=''; log.style.display='block'; }
  bcSetCardState(id,'info','⏳ جارٍ النشر...');
  fetch('admin.php?ajax=broadcast_deploy_one', {
    method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({id:id})
  }).then(function(r){return r.json();}).then(function(d){
    if (d.steps) d.steps.forEach(function(s){
      bcSetCardState(id, s.ok?'ok':'err', s.msg);
    });
    if (d.ok) {
      var card = document.getElementById('bc-card-'+id);
      if (card) { card.classList.remove('pending'); card.classList.add('deployed'); }
      var badge = card ? card.querySelector('.bc-badge') : null;
      if (badge) { badge.textContent='نُشر'; badge.className='bc-badge ok'; }
    }
    bcSetCardState(id, d.ok?'ok':'err', d.ok ? '✅ اكتمل النشر بنجاح' : '⚠ اكتمل مع أخطاء');
  }).catch(function(e){ bcSetCardState(id,'err','❌ خطأ في الاتصال: '+e.message); });
}

function bcDeploySelected() {
  var ids = bcSelectedIds();
  if (!ids.length) { alert('حدّد دوماناً واحداً على الأقل'); return; }
  var prog = document.getElementById('bc-progress');
  var bar  = document.getElementById('bc-progress-bar');
  prog.style.display='block'; bar.style.width='0%';
  var done = 0;
  function next() {
    if (!ids.length) { bar.style.width='100%'; return; }
    var id = ids.shift();
    bcDeployOne(id);
    done++;
    bar.style.width = Math.round(done/(done+ids.length)*100) + '%';
    setTimeout(next, 1200); // stagger to avoid hammering cPanel
  }
  next();
}

function bcIndexNowOne(id) {
  bcSetCardState(id,'info','⏳ إرسال إلى IndexNow...');
  fetch('admin.php?ajax=broadcast_indexnow',{
    method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({ids:[id]})
  }).then(function(r){return r.json();}).then(function(d){
    if (d.ok) { bcSetCardState(id,'ok','✅ تم إرسال IndexNow (كود: '+(d.results&&d.results[0]?d.results[0].code:'OK')+')');}
    else { bcSetCardState(id,'err','❌ '+(d.error||'فشل IndexNow')); }
  });
}

function bcIndexNowSelected() {
  var ids = bcSelectedIds();
  if (!ids.length) { alert('حدّد دوماناً واحداً على الأقل'); return; }
  var gl = document.getElementById('bc-global-log');
  gl.innerHTML = '<div style="color:var(--muted);font-size:13px">⏳ إرسال IndexNow لـ '+ids.length+' دومين...</div>';
  fetch('admin.php?ajax=broadcast_indexnow',{
    method:'POST',headers:{'Content-Type':'application/json'},
    ...
    (d.results||[]).forEach(function(r){
      var ln = document.createElement('div');
      ln.style.cssText='font-size:12px;padding:4px 0;border-bottom:1px solid var(--border)';
      ln.style.color = r.ok ? '#22c55e' : '#ef4444';
      ln.textContent = (r.ok?'✅ ':'❌ ') + r.domain + ' — HTTP ' + r.code;
      gl.appendChild(ln);
    });
  });
}

function bcPingSitemaps() {
  var ids = bcSelectedIds();
  if (!ids.length) { alert('حدّد دوماناً واحداً على الأقل'); return; }
  var gl = document.getElementById('bc-global-log');
  gl.innerHTML = '<div style="color:var(--muted);font-size:13px">⏳ إرسال Ping لـ Google & Bing...</div>';
  fetch('admin.php?ajax=broadcast_ping_sitemaps',{
    method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({ids:ids})
  }).then(function(r){return r.json();}).then(function(d){
    gl.innerHTML = '';
    (d.results||[]).forEach(function(r){
      ...
      ln.style.cssText='font-size:12px;padding:4px 0;border-bottom:1px solid var(--border)';
      ln.style.color = r.ok ? '#22c55e' : '#ef4444';
      ln.textContent = (r.ok?'✅ ':'⚠ ') + r.domain + ' — Google:'+r.google+' Bing:'+r.bing;
      gl.appendChild(ln);
    });
  });
}
...
try {
    $smDomains = $pdo->query(
        "SELECT id, full_domain, category_slug, status, doc_root, force_https, last_deployed_at
         FROM domains ORDER BY created_at DESC"
    )->fetchAll(PDO::FETCH_ASSOC);
} catch(\Throwable $e) { $smDomains = []; }

$mainDomHost = parse_url(rtrim(get_cfg($pdo,'site_url', defined('SITE_URL')?SITE_URL:''), '/'), PHP_URL_HOST) ?: 'yassota.com';
$cats = [];
try { $cats = $pdo->query("SELECT slug, name FROM categories WHERE slug IS NOT NULL AND slug<>'' ORDER BY name ASC")->fetchAll(PDO::FETCH_ASSOC); } catch(\Throwable $e) {}

// Predefined quick-create subdomains
$quickSubs = [
    ['sub'=>'apps',     'icon'=>'📱', 'desc'=>'جميع التطبيقات'],
    ['sub'=>'games',    'icon'=>'🎮', 'desc'=>'الألعاب'],
    ['sub'=>'tools',    'icon'=>'🛠️',  'desc'=>'الأدوات'],
    ['sub'=>'android',  'icon'=>'🤖', 'desc'=>'تطبيقات أندرويد'],
    ['sub'=>'apk',      'icon'=>'📦', 'desc'=>'ملفات APK'],
    ['sub'=>'download',  'icon'=>'⬇️', 'desc'=>'التحميل'],
    ['sub'=>'free',     'icon'=>'🆓', 'desc'=>'مجاني'],
    ['sub'=>'top',      'icon'=>'🏆', 'desc'=>'الأكثر تحميلاً'],
    ['sub'=>'new',      'icon'=>'🆕', 'desc'=>'الأحدث'],
    ['sub'=>'best',     'icon'=>'⭐', 'desc'=>'الأفضل'],
    ['sub'=>'pro',      'icon'=>'💎', 'desc'=>'احترافي'],
    ['sub'=>'vip',      'icon'=>'👑', 'desc'=>'مميز'],
    ['sub'=>'hub',      'icon'=>'🌐', 'desc'=>'مركز'],
    ['sub'=>'store',    'icon'=>'🛍️',  'desc'=>'المتجر'],
    ['sub'=>'market',   'icon'=>'🏪', 'desc'=>'السوق'],
    ['sub'=>'mod',      'icon'=>'⚙️',  'desc'=>'تطبيقات مُعدَّلة'],
    ['sub'=>'premium',  'icon'=>'💰', 'desc'=>'مدفوعة مجاناً'],
    ['sub'=>'lite',     'icon'=>'⚡', 'desc'=>'إصدارات خفيفة'],
];
$existingSubNames = array_map(fn($d) => explode('.', $d['full_domain'])[0], $smDomains);
?>
<style>
.sm-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:12px;margin:18px 0}
.sm-quick{background:var(--surface);border:1.5px solid var(--border);border-radius:12px;padding:14px;text-align:center;cursor:pointer;transition:border-color .2s,background .2s;position:relative}
.sm-quick:hover{border-color:var(--primary);background:rgba(59,130,246,.05)}
.sm-quick.exists{border-color:rgba(34,197,94,.5);background:rgba(34,197,94,.05)}
.sm-quick .sm-icon{font-size:28px;margin-bottom:6px}
.sm-quick .sm-name{font-family:monospace;font-weight:800;font-size:13px;color:var(--primary)}
.sm-quick .sm-desc{font-size:11px;color:var(--muted);margin-top:2px}
.sm-quick .sm-domain{font-size:10px;color:var(--muted);font-family:monospace;margin-top:4px;word-break:break-all}
.sm-quick .sm-badge{position:absolute;top:8px;left:8px;font-size:9px;padding:2px 6px;border-radius:10px;font-weight:700}
.sm-badge.done{background:rgba(34,197,94,.2);color:#22c55e}
.sm-badge.pending{background:rgba(245,158,11,.2);color:#f59e0b}
.sm-form{background:var(--surface);border:1.5px solid var(--border);border-radius:14px;padding:20px}
.sm-table th{font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;padding:8px 10px;border-bottom:1px solid var(--border)}
.sm-table td{padding:10px;border-bottom:1px solid var(--border);font-size:13px;vertical-align:middle}
.sm-table tr:last-child td{border-bottom:none}
.sm-status{font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px}
.sm-status.active{background:rgba(34,197,94,.15);color:#22c55e}
.sm-status.pending{background:rgba(245,158,11,.15);color:#f59e0b}
.sm-log{margin-top:10px;padding:10px;background:var(--bg);border-radius:8px;font-family:monospace;font-size:11px;display:none;max-height:150px;overflow-y:auto}
</style>

<div class="admin-header">
  <h1>🌐 إنشاء دومينات فرعية حقيقية</h1>
  <p style="color:var(--muted);font-size:13px;margin-top:4px">
    أنشئ دومينات فرعية حقيقية على استضافتك — كل واحد يعمل كمتجر تطبيقات مستقل يُفهرَس بشكل منفصل ويقود للموقع الأم
  </p>
</div>
...
<div class="panel">
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;flex-wrap:wrap;gap:10px">
    <h3 style="margin:0;font-size:15px">⚡ إنشاء سريع — اضغط لإنشاء دومين فرعي</h3>
    <div style="display:flex;gap:8px;align-items:center">
      <label style="display:flex;align-items:center;gap:5px;font-size:12px;cursor:pointer">
        <input type="checkbox" id="sm-opt-ssl" checked> SSL تلقائي
      </label>
      <label style="display:flex;align-items:center;gap:5px;font-size:12px;cursor:pointer">
        <input type="checkbox" id="sm-opt-https" checked> فرض HTTPS
      </label>
      <label style="display:flex;align-items:center;gap:5px;font-size:12px;cursor:pointer">
        <input type="checkbox" id="sm-opt-deploy" checked> نشر فوري
      </label>
    </div>
  </div>

  <div class="sm-grid">
    <?php foreach ($quickSubs as $qs):
      $exists = in_array($qs['sub'], $existingSubNames);
      $domFull = $qs['sub'] . '.' . $mainDomHost;
    ?>
    <div class="sm-quick <?= $exists ? 'exists' : '' ?>" onclick="smCreateQuick('<?= $qs['sub'] ?>')">
      <?php if ($exists): ?><span class="sm-badge done">✅ موجود</span><?php endif; ?>
      <div class="sm-icon"><?= $qs['icon'] ?></div>
      <div class="sm-name"><?= $qs['sub'] ?></div>
      <div class="sm-desc"><?= $qs['desc'] ?></div>
      <div class="sm-domain"><?= $domFull ?></div>
    </div>
    <?php endforeach; ?>
  </div>
  <div id="sm-quick-log" class="sm-log"></div>
</div>

<!-- ── Custom create form ── -->
<div class="sm-form" style="margin-top:20px">
  <h3 style="margin:0 0 16px;font-size:15px">➕ إنشاء دومين فرعي مخصص</h3>
  <div style="display:grid;grid-template-columns:1fr 1fr auto;gap:12px;align-items:end;flex-wrap:wrap">
    <div>
      <label style="font-size:12px;color:var(--muted);display:block;margin-bottom:5px">اسم الدومين الفرعي *</label>
      <div style="display:flex;align-items:center;border:1.5px solid var(--border);border-radius:8px;overflow:hidden;background:var(--bg)">
        <input type="text" id="sm-custom-sub" placeholder="apps" style="flex:1;padding:10px;border:none;background:transparent;color:var(--text);font-family:monospace;font-size:14px">
        <span style="padding:10px;color:var(--muted);font-size:13px;font-family:monospace;border-right:1px solid var(--border);white-space:nowrap">.<?= h($mainDomHost) ?></span>
      </div>
    </div>
    <div>
      <label style="font-size:12px;color:var(--muted);display:block;margin-bottom:5px">القسم (اختياري)</label>
      <select id="sm-custom-cat" style="width:100%;padding:10px;border:1.5px solid var(--border);border-radius:8px;background:var(--bg);color:var(--text);font-size:13px">
        <option value="">جميع التطبيقات</option>
        <?php foreach ($cats as $c): ?>
        <option value="<?= h($c['slug']) ?>"><?= h($c['name'] ?: $c['slug']) ?></option>
        <?php endforeach; ?>
      </select>
    </div>
    <button onclick="smCreateCustom()" class="btn-primary" style="padding:10px 24px;white-space:nowrap">إنشاء الآن</button>
  </div>
  <div style="display:flex;gap:16px;margin-top:10px;flex-wrap:wrap">
    <label style="display:flex;align-items:center;gap:6px;font-size:12px;cursor:pointer">
      <input type="checkbox" id="sm-c-ssl" checked> تفعيل SSL (AutoSSL)
    </label>
    <label style="display:flex;align-items:center;gap:6px;font-size:12px;cursor:pointer">
      <input type="checkbox" id="sm-c-https" checked> فرض HTTPS (301 Redirect)
    </label>
    <label style="display:flex;align-items:center;gap:6px;font-size:12px;cursor:pointer">
      <input type="checkbox" id="sm-c-deploy" checked> نشر موقع المتجر فوراً
    </label>
  </div>
  <div id="sm-custom-log" class="sm-log"></div>
</div>

<!-- ── Existing domains table ── -->
<div class="panel" style="margin-top:20px">
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;flex-wrap:wrap;gap:10px">
    <h3 style="margin:0;font-size:15px">📋 الدومينات المسجلة (<?= count($smDomains) ?>)</h3>
    <div style="display:flex;gap:8px">
      <button onclick="smSyncCpanel()" style="padding:8px 14px;background:var(--bg);border:1px solid var(--border);border-radius:8px;font-size:12px;cursor:pointer;color:var(--text)">
        🔄 مزامنة من cPanel
      </button>
      <a href="admin.php?page=domain-broadcast" style="padding:8px 14px;background:var(--primary);color:#fff;border-radius:8px;font-size:12px;cursor:pointer;text-decoration:none">
        📡 لوحة النشر
      </a>
    </div>
  </div>

  <?php if (empty($smDomains)): ?>
  ...
  <table class="sm-table" style="width:100%;border-collapse:collapse">
    <thead>
      <tr>
        ...
        <th style="text-align:right">HTTPS</th>
        <th style="text-align:right">آخر نشر</th>
        <th style="text-align:right">إجراءات</th>
      </tr>
    </thead>
    <tbody>
    <?php foreach ($smDomains as $dom):
      $deployed = !empty($dom['last_deployed_at']);
    ...
    <tr id="sm-row-<?= $dom['id'] ?>">
      <td>
        ...
        <?php if($dom['doc_root']): ?>
        <div style="font-size:10px;color:var(--muted);font-family:monospace"><?= h($dom['doc_root']) ?></div>
        <?php endif; ?>
      </td>
      <td><?= $dom['category_slug'] ? h($dom['category_slug']) : '<span style="color:var(--muted)">جميع</span>' ?></td>
      <td><span class="sm-status <?= $deployed?'active':'pending' ?>"><?= $deployed?'نُشر':'لم يُنشر' ?></span></td>
      <td>
        <label style="display:flex;align-items:center;gap:5px;cursor:pointer;font-size:12px">
          <input type="checkbox" <?= $https?'checked':'' ?> onchange="smSetHttps(<?= $dom['id'] ?>,this.checked)">
          فرض HTTPS
        </label>
      </td>
      <td style="color:var(--muted);font-size:12px"><?= $dom['last_deployed_at'] ? date('Y-m-d H:i', strtotime($dom['last_deployed_at'])) : '—' ?></td>
      <td>
        <div style="display:flex;gap:6px;flex-wrap:wrap">
...
<!-- ── cPanel sync result ── -->
<div id="sm-cpanel-result" style="display:none;margin-top:16px" class="panel">
  <h4 style="margin:0 0 10px;font-size:13px">📋 دومينات فرعية في cPanel</h4>
  <div id="sm-cpanel-list"></div>
</div>

<script>
var SM_MAIN_DOM = <?= json_encode($mainDomHost) ?>;

function smLog(el, msg, ok) {
  var d = document.getElementById(el);
  if (!d) return;
  d.style.display = 'block';
  var ln = document.createElement('div');
  ln.style.cssText = 'padding:2px 0;color:' + (ok===true?'#22c55e':ok===false?'#ef4444':'var(--muted)');
  ln.textContent = msg;
  d.appendChild(ln);
...
function smDoCreate(sub, cat, ssl, https, deploy, logEl) {
  var d = document.getElementById(logEl);
  if (d) { d.innerHTML=''; d.style.display='block'; }
  smLog(logEl, '⏳ جارٍ إنشاء ' + sub + '.' + SM_MAIN_DOM + '...', null);
  fetch('admin.php?ajax=broadcast_create_subdomain', {
    method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({subdomain:sub, category_slug:cat, ssl:ssl, force_https:https, deploy:deploy})
  }).then(function(r){return r.json();}).then(function(d){
    (d.steps||[]).forEach(function(s){ smLog(logEl, s.msg, s.ok); });
    if (d.ok) {
      smLog(logEl, '✅ اكتملت العملية — ' + d.domain, true);
      setTimeout(function(){ location.reload(); }, 2000);
    } else {
      smLog(logEl, '⚠ اكتملت مع أخطاء', false);
    }
  }).catch(function(e){ smLog(logEl,'❌ خطأ: '+e.message, false); });
}

function smCreateQuick(sub) {
  var ssl    = document.getElementById('sm-opt-ssl').checked;
  var https  = document.getElementById('sm-opt-https').checked;
  var deploy = document.getElementById('sm-opt-deploy').checked;
  smDoCreate(sub, '', ssl, https, deploy, 'sm-quick-log');
}

function smCreateCustom() {
  var sub  = document.getElementById('sm-custom-sub').value.trim().toLowerCase().replace(/[^a-z0-9\-]/g,'');
  var cat  = document.getElementById('sm-custom-cat').value;
  if (!sub) { alert('أدخل اسم الدومين الفرعي'); return; }
  var ssl    = document.getElementById('sm-c-ssl').checked;
  var https  = document.getElementById('sm-c-https').checked;
  var deploy = document.getElementById('sm-c-deploy').checked;
  smDoCreate(sub, cat, ssl, https, deploy, 'sm-custom-log');
}

function smDeployOne(id) {
  var logEl = 'sm-row-log-'+id;
  var d = document.getElementById(logEl); if(d){d.innerHTML='';d.style.display='block';}
  smLog(logEl,'⏳ جارٍ النشر...', null);
  fetch('admin.php?ajax=broadcast_deploy_one', {
    method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({id:id})
  }).then(function(r){return r.json();}).then(function(d){
    (d.steps||[]).forEach(function(s){ smLog(logEl, s.msg, s.ok); });
    smLog(logEl, d.ok ? '✅ اكتمل النشر' : '⚠ مع أخطاء', d.ok);
    if (d.ok) { var row=document.getElementById('sm-row-'+id); if(row) row.querySelector('.sm-status').textContent='نُشر'; }
  }).catch(function(e){ smLog(logEl,'❌ '+e.message,false); });
}

function smSetHttps(id, val) {
  fetch('admin.php?ajax=broadcast_set_https', {
    method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({id:id, force_https:val})
  }).then(function(r){return r.json();}).then(function(d){
    if (!d.ok) alert('فشل تحديث HTTPS: ' + (d.error||''));
  });
}

function smSyncCpanel() {
  var res = document.getElementById('sm-cpanel-result');
  var list = document.getElementById('sm-cpanel-list');
  res.style.display='none';
  fetch('admin.php?ajax=broadcast_list_cpanel_subs')
    .then(function(r){return r.json();}).then(function(d){
      if (!d.ok) { alert('cPanel: ' + (d.error||'فشل')); return; }
      res.style.display='block';
      if (!d.subdomains || !d.subdomains.length) { list.innerHTML='<p style="color:var(--muted);font-size:13px">لا توجد دومينات فرعية في cPanel</p>'; return; }
      list.innerHTML = d.subdomains.map(function(s){
        return '<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border);font-size:13px">' +
          '<span style="font-family:monospace">' + (s.domain||s.subdomain||JSON.stringify(s)) + '</span>' +
          '<span style="color:var(--muted);font-size:11px">' + (s.documentroot||s.dir||'') + '</span>' +
          '</div>';
      }).join('');
...
<!-- ── Domain Auto-Setup Wizard ── -->
<?php
$cpUser  = get_cfg($pdo, 'cpanel_user', 'yassqfkf');
$cpToken = get_cfg($pdo, 'cpanel_api_token', '') ?: get_cfg($pdo, 'cpanel_api_key', '');
$allDomains = [];
try { $allDomains = $pdo->query("SELECT full_domain,doc_root,status,site_mode,created_at FROM domains ORDER BY created_at DESC")->fetchAll(); } catch(\Throwable $e) {}
?>
<div class="panel" style="margin-bottom:20px;border-right:3px solid var(--primary)">
  <h3 style="margin:0 0 16px;font-size:15px">
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:middle;margin-left:6px"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
    إضافة دومين وإعداده تلقائياً
  </h3>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px">
    <div>
      <label class="form-label">اسم الدومين (مثال: mysite.com)</label>
      <input id="hm-domain-name" class="form-input" placeholder="mysite.com" style="direction:ltr">
    </div>
    <div>
      <label class="form-label">المسار التلقائي</label>
      <input id="hm-domain-path" class="form-input" readonly style="direction:ltr;background:var(--bg);color:var(--muted);font-family:monospace;font-size:12px" value="/home/<?= h($cpUser) ?>/public_html/">
    </div>
    <div>
      <label class="form-label">وضع الموقع</label>
      <select id="hm-site-mode" class="form-input">
        <option value="mirror">🪞 نسخة طبق الأصل من yassota</option>
        <option value="redirect">↩ إعادة توجيه للموقع الرئيسي</option>
        <option value="category">📂 قسم محدد فقط</option>
        <option value="standalone">🏠 موقع مستقل</option>
      </select>
    </div>
    <div>
      <label class="form-label">خيارات تلقائية</label>
      <div style="display:flex;flex-direction:column;gap:6px;padding:10px;background:var(--bg);border-radius:8px;border:1px solid var(--border)">
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer">
          <input type="checkbox" id="hm-auto-index" checked style="width:15px;height:15px"> تثبيت index.php تلقائياً
        </label>
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer">
          <input type="checkbox" id="hm-auto-https" checked style="width:15px;height:15px"> تفعيل HTTPS تلقائياً
        </label>
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer">
          <input type="checkbox" id="hm-shared-perms" checked style="width:15px;height:15px"> صلاحيات مشتركة (الوصول لملفات الدومينات الأخرى)
        </label>
      </div>
    </div>
  </div>
  <button onclick="autoSetupDomain()" class="btn-save" style="padding:10px 20px">
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:middle;margin-left:6px"><path d="M12 5v14M5 12h14"/></svg>
    إضافة وإعداد تلقائياً
  </button>
  <div id="hm-setup-log" style="margin-top:12px;font-size:13px"></div>
</div>

<!-- ── File Structure View /home/yassqfkf/ ── -->
<div class="panel" style="margin-bottom:20px">
  <h3 style="margin:0 0 14px;font-size:15px">
    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:middle;margin-left:6px"><path d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"/></svg>
    هيكل الملفات — /home/<?= h($cpUser) ?>/
  </h3>
  <div style="background:#0f172a;border-radius:10px;padding:16px;font-family:monospace;font-size:12px;color:#94a3b8;overflow-x:auto;direction:ltr">
    <div style="color:#38bdf8">/home/<?= h($cpUser) ?>/</div>
    <div style="padding-right:16px;margin-top:4px">
      <div style="color:#4ade80">├── public_html/         ← الموقع الرئيسي (<?= h($baseDomain) ?>)</div>
      <?php foreach ($allDomains as $d):
        $folder = preg_replace('#^https?://#i', '', $d['full_domain']);
        $status_color = $d['status'] === 'active' ? '#4ade80' : '#f59e0b';
      ?>
      <div style="color:<?= $status_color ?>">├── <?= h($folder) ?>/  ← <?= h($d['site_mode'] ?? 'redirect') ?></div>
      <?php endforeach; ?>
      <?php if (empty($allDomains)): ?>
      <div style="color:#64748b">└── (لا توجد دومينات مضافة بعد)</div>
      <?php else: ?>
  ...
  <div style="margin-top:14px;overflow-x:auto">
    <table class="admin-table" style="min-width:500px">
      <thead><tr><th>الدومين</th><th>المسار</th><th>الوضع</th><th>الحالة</th></tr></thead>
      <tbody>
      <?php foreach ($allDomains as $d):
        $stC=['active'=>'#22c55e','pending'=>'#f59e0b','paused'=>'#ef4444'];
        $stL=['active'=>'نشط','pending'=>'معلق','paused'=>'متوقف'];
      ?>
        <tr>
          <td style="font-family:monospace;font-size:12px;direction:ltr"><?= h($d['full_domain']) ?></td>
          <td style="font-family:monospace;font-size:11px;color:var(--muted);direction:ltr"><?= h($d['doc_root'] ?: '—') ?></td>
          <td style="font-size:12px"><?= h($d['site_mode'] ?? '—') ?></td>
          <td><span style="font-size:12px;font-weight:600;color:<?= $stC[$d['status']] ?? '#94a3b8' ?>"><?= $stL[$d['status']] ?? h($d['status']) ?></span></td>
        </tr>
      <?php endforeach; ?>
      </tbody>
    </table>
...
<div class="panel" style="margin-bottom:20px">
  <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
    ...
    <button id="https-toggle-btn" onclick="toggleHttps()" class="btn-save" style="background:<?= $httpsEnabled ? '#ef4444' : '#22c55e' ?>;min-width:150px">
...
<script>
function autoSetupDomain() {
  var domain = document.getElementById('hm-domain-name').value.trim().toLowerCase().replace(/^https?:\/\//,'');
  if (!domain || !domain.includes('.')) { alert('أدخل اسم دومين صحيح مثل: mysite.com'); return; }
  var mode     = document.getElementById('hm-site-mode').value;
  var autoIdx  = document.getElementById('hm-auto-index').checked;
  var autoHtps = document.getElementById('hm-auto-https').checked;
  var sharedP  = document.getElementById('hm-shared-perms').checked;
  var log      = document.getElementById('hm-setup-log');
  log.innerHTML = '<div style="color:var(--muted)">⏳ جارٍ إعداد الدومين...</div>';
  // Auto-fill path
  var cpUser = '<?= h($cpUser) ?>';
  document.getElementById('hm-domain-path').value = '/home/'+cpUser+'/'+domain+'/';
  fetch('admin.php?ajax=auto_setup_domain', {
    method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({domain:domain, mode:mode, auto_index:autoIdx, auto_https:autoHtps, shared_perms:sharedP})
  }).then(function(r){return r.json();})
  .then(function(d){
    if (d.ok) {
      var steps = (d.steps||[]).map(function(s){ return '<div style="font-size:12px;padding:4px 0;color:'+(s.ok?'#22c55e':'#ef4444')+'">'+( s.ok?'✅':'❌')+' '+s.msg+'</div>'; }).join('');
      log.innerHTML = '<div style="color:#22c55e;font-weight:600;margin-bottom:8px">✅ تم إعداد '+domain+' بنجاح!</div>'+steps+'<div style="margin-top:8px"><a href="https://'+domain+'/" target="_blank" style="color:var(--primary);font-size:12px">افتح الدومين ←</a></div>';
      setTimeout(function(){location.reload();},2000);
    } else {
      log.innerHTML = '<div style="color:#ef4444">❌ '+(d.error||'فشل الإعداد')+'</div>';
    }
  }).catch(function(){ log.innerHTML='<div style="color:#ef4444">❌ فشل الاتصال</div>'; });
}
document.addEventListener('DOMContentLoaded', function() {
  var inp = document.getElementById('hm-domain-name');
  if (inp) inp.addEventListener('input', function() {
    var v = this.value.trim().toLowerCase().replace(/^https?:\/\//,'');
    document.getElementById('hm-domain-path').value = v ? '/home/<?= h($cpUser) ?>/'+v+'/' : '/home/<?= h($cpUser) ?>/public_html/';
  });
});
...
/* ════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════ */
$seoApps = $pdo->query(
    "SELECT id, name, slug, seo_title, meta_description, long_description, icon_path, status,
...
function yai_seo_score(array $app): array {
    ...
    $desc   = trim($app['meta_description'] ?? '');
    ...
    if (!$title)           { $score -= 20; $issues[] = ['sr'=>'critical','msg'=>'عنوان SEO مفقود']; }
