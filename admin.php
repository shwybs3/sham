...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'broadcast_deploy_one' && is_admin()) {
    ...
    $cpCfg = [
        'url'   => rtrim(get_cfg($pdo,'cpanel_url',''), '/') ?: 'https://localhost:2083',
        'user'  => get_cfg($pdo,'cpanel_user',''),
        'token' => get_cfg($pdo,'cpanel_api_token',''),
    ];
    ...
    /* ── 1. Generate index.php ── */
    $catLine  = $catSlug ? "var_export('".addslashes($catSlug)."')" : "''";
    $indexPhp = <<<'PHPEOF'
<?php
/*  Satellite site — served by domain-broadcast  */
$_parentCfg = dirname($_SERVER['DOCUMENT_ROOT']);
foreach ([$_parentCfg.'/config.local.php', $_SERVER['DOCUMENT_ROOT'].'/config.local.php'] as $_f)
    if (file_exists($_f)) { require_once $_f; break; }
foreach ([$_parentCfg.'/config.php', $_SERVER['DOCUMENT_ROOT'].'/config.php'] as $_f)
    if (file_exists($_f)) { require_once $_f; break; }
if (!isset($pdo)) { include dirname($_SERVER['DOCUMENT_ROOT']).'/index.php'; exit; }
PHPEOF;
    // Append the domain-template file contents (already has full logic + CSS)
    $tplPath = __DIR__ . '/domain-template/index.php';
    $tplBody = file_exists($tplPath) ? file_get_contents($tplPath) : '<?php echo "Template missing"; ?>';
    // Strip the bootstrap section from template (it has its own at the top)
    $tplBody = preg_replace('/^<\?php.*?if \(!isset\(\$pdo\)\).*?die.*?$/ms', '', $tplBody, 1);
    $indexPhp .= "\n" . $tplBody;
    ...
    /* ── 2. Generate sitemap.php ── */
    $sitemapPhp = '<?php' . "\n" .
'// Satellite sitemap — auto-generated by domain-broadcast' . "\n" .
'$_p=dirname($_SERVER["DOCUMENT_ROOT"]);' . "\n" .
'foreach([$_p."/config.local.php",$_SERVER["DOCUMENT_ROOT"]."/config.local.php"] as $_f)if(file_exists($_f)){require_once $_f;break;}' . "\n" .
'foreach([$_p."/config.php",$_SERVER["DOCUMENT_ROOT"]."/config.php"] as $_f)if(file_exists($_f)){require_once $_f;break;}' . "\n" .
'if(!isset($pdo)){header("HTTP/1.0 503 Service Unavailable");exit;}' . "\n" .
'header("Content-Type: application/xml; charset=utf-8");' . "\n" .
'$domUrl="https://".($_SERVER["HTTP_HOST"]??"'.$fullDom.'");' . "\n" .
...
'</urlset>';
    ...
    $steps[] = ['step'=>'.htaccess','ok'=>$r4['ok'],'msg'=>$r4['ok']?"✅ .htaccess نُشر":'❌ '.$r4['error']];
