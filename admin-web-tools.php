...
// ── Delete Tool ──
if ($action === 'delete' && $id && hash_equals($_SESSION['csrf'] ?? '', $_GET['t'] ?? '')) {
    try {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !csrf_check()) {
    $saveError = 'انتهت صلاحية الجلسة قبل الحفظ — أعد تحميل الصفحة وحاول مجدداً.';
}
if ($_SERVER['REQUEST_METHOD'] === 'POST' && csrf_check()) {
...
// إن فُتح هذا الملف مباشرة (رابط مستقل) بدون meta viewport، يعرضه المتصفح
// بعرض سطح مكتب افتراضي (~980px) ثم يصغّره، فتتوقف @media (max-width:...)
// عن العمل ويظهر الشكل مزدحمًا وغير متناسق على الجوال.
$wtDirectAccess = basename($_SERVER['SCRIPT_FILENAME'] ?? '') === basename(__FILE__);
if ($wtDirectAccess): ?>
<!DOCTYPE html>
...
<div class="wt-tools-admin">
  <?php if ($saveError): ?>
  <div style="background:#fef2f2;border:1px solid #fca5a5;color:#b91c1c;padding:12px 16px;border-radius:8px;margin-bottom:16px;font-size:14px">⚠️ <?= h($saveError) ?></div>
  <?php endif; ?>
  <div class="wt-topbar">
